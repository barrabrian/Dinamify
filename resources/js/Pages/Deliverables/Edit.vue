    ...
    <form @submit.prevent="update">
        ...
        <div class="shadow-lg bg-gray-100 border border-gray-300 rounded" style="min-height: calc(100vh - 150px - 3rem);">
            <div class="p-12 bg-white flex justify-between items-center">
                ...
                <trashed-message v-if="deliverable.deleted_at" class="mb-6" @restore="restore"> Este entregável foi deletado. </trashed-message>

                <div class="flex gap-8">
                    <button v-if="!deliverable.deleted_at" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Deletar</button>
                ...
                <div class="col-span-3 ">
                    ...
                    <div class="w-100 bg-gray-50 rounded-md shadow border border-gray-200 ">
                        <div class="flex justify-between items-center py-2 px-3 border-b">
                            ...
                            <div id="text-helper" class="text-xs italic text-gray-400 cursor-pointer" @click="showHelp = true">
                                Ajuda
                            </div>
                        </div>
                        <div class="py-2 px-4 bg-white rounded-b-lg dark:bg-gray-800">
                            <textarea v-model="form.html" :error="form.errors.html" rows="20" class="block px-0 w-full text-sm text-gray-800 bg-white border-0 focus:ring-0 h-full m-0" placeholder="Crie seu pdf em HTML aqui..." required @keyup="preview" @keydown.tab.prevent.exact @keydown.ctrl.s.stop.prevent="update" @keydown.meta.s.stop.prevent="update"/>
                        </div>
                    </div>
                </div>
        ...
        <sidebar-modal :show="showHF" @close="showHF = false">
            <div>
                ...
                <div class="flex flex-col border-dashed border-2 border-gray-300 rounded bg-gray-100 p-4">
                    <text-area-input v-model="form.header" :error="form.errors.header" rows="12" class="pb-4 w-full " placeholder="Crie o cabeçalho em HTML aqui..." label="Cabeçalho" />
                </div>
                <div class="flex mt-4 flex-col border-dashed border-2 border-gray-300 rounded bg-gray-100 p-4">
                    <text-area-input v-model="form.footer" :error="form.errors.footer" rows="12" class="pb-4 w-full " placeholder="Crie o rodapé em HTML aqui..." label="Rodapé" />
                </div>
...
export default {
  ...
  data() {
    return {
      form: this.$inertia.form({
        ...
        html: this.deliverable.html,
        header: this.deliverable.header,
        footer: this.deliverable.footer,
        form_id: this.deliverable.form_id,
      }),
      variables: {},
      alternatives: {},
      condition: {
  ...
  mounted() {
    this.preview();
    axios.get('/api/questions?form_id=' + this.form.form_id)
    .then (response => {
        this.variables = response.data;
    });
    setTimeout(() => {
  ...
  methods: {
    update() {
      this.form.put(`/deliverables/${this.deliverable.id}`)
    },
    destroy() {
      if (confirm('Tem certeza que deseja deletar este entregável?')) {
        this.$inertia.delete(`/deliverables/${this.deliverable.id}`)
      }
    },
    restore() {
      if (confirm('Tem certeza que deseja restaurar este entregável?')) {
        this.$inertia.put(`/deliverables/${this.deliverable.id}/restore`)
      }
    },
    preview() {
        setTimeout(() => {
