...
class AutomationsController extends Controller
{
    ...
    {
        $flow = Auth::user()->account->automations()->create(
            Request::validate([
    ...
    public function edit(Automation $flow)
    {
        return Inertia::render('Flows/Edit', [
            'flow' => [
                ...
                'email_question_id' => $flow->email_question_id,
                'deleted_at' => $flow->deleted_at,
            ],
            'typeform_plugins' => Auth::user()->account->typeform_plugins()
                ->orderBy('name')
            ...
            'deliverables' => Auth::user()->account->deliverables()
                ->orderBy('name')
                ->get(),
            'ebooks' => Auth::user()->account->ebooks()->where('name', $flow->deliverable->name)
                ->orderBy('id','DESC')
                ->get()
                ->transform(fn ($ebook) => [
                    'id' => $ebook->id,
                    'name' => $ebook->name,
                    'email' => $flow->email_question->answers()->where('answer_id', $ebook->response_id)->first()->value,
